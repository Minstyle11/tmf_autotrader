...
python3 - <<'PY'
# SKIP_EXEC_MARKET_CLOSED: treat holiday/offsession market-closed as PASS (regression only)
import sys
try:
    ...
    msg = str(e)
    if "EXEC_MARKET_CLOSED" in msg:
        print("[SKIP] EXEC_MARKET_CLOSED (holiday/offsession) -> treat taifex split regression as PASS")
        sys.exit(0)
    raise
